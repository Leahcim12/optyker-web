...
function storeForDate(d){var wd=((d.getDay()+6)%7)+1;return (S.data.store_hours||[]).find(function(x){return +x.weekday===wd})||{weekday:wd,active:false}}
...
function storeGrid(){var days=daysInMonth(S.month),h='<div class="oa16Grid oa16StoreGrid" style="grid-template-columns:'+gridCols(days)+'">'+headRow(days)+'<div class="oa16RowHead">Turni</div>';for(var i=1;i<=days;i++){var d=parseMonth(S.month);d.setDate(i);var sh=storeForDate(d),today=ymd(d)===ymd(new Date());h+='<div class="oa16Cell '+(sh.active===false?'closed':'default')+' '+(today?'oa16TodayCol':'')+'">'+(sh.active===false?'×':'<div class="times">'+fmtStore(sh)+'</div>')+'</div>'}return h+'</div>'}
...
function mount(){
  ...
  E('oa16Export').onclick=exportCsv;
  return true
}
function render(){if(!S.data)return;E('oa16StoreWrap').innerHTML=storeGrid();E('oa16UsersWrap').innerHTML=userGrid();E('oa16UsersWrap').querySelectorAll('.oa16Cell[data-op]').forEach(function(c){c.onclick=function(){openEdit(c.dataset.op,c.dataset.date)}})}
function load(){
...
function exportCsv(){
  ...
  var csv=rows.map(function(r){return r.map(function(v){return '"'+String(v==null?'':v).replace(/"/g,'""')+'"'}).join(';')}).join('\n'),blob=new Blob([String.fromCharCode(65279)+csv],{type:'text/csv;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='turni-'+S.month+'.csv';a.click();setTimeout(function(){URL.revokeObjectURL(a.href)},500)
}
function bindEditor(){
...
for req in [MARK,'Orari punto vendita','Orari utenti','Permesso','Malattia','Esporta CSV','optyker_staff_schedule_api']:
