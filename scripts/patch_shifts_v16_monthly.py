...
function userGrid(){var days=daysInMonth(S.month),ops=(S.data.operators||[]).filter(function(o){return String(o&&o.username||'').trim().toUpperCase()!=='OTTICA VISUAL CARE'}),h='<div class="oa16Grid oa16UsersGrid" style="grid-template-columns:'+gridCols(days)+'">'+headRow(days);ops.forEach(function(o){var op=o.username;h+='<div class="oa16RowHead">'+X(op)+'</div>';for(var i=1;i<=days;i++){var d=parseMonth(S.month);d.setDate(i);var date=ymd(d),sh=storeForDate(d),sc=schedule(op,date),cl=cellClass(sc,sh),today=date===ymd(new Date());h+='<div class="oa16Cell clickable '+cl+' '+(today?'oa16TodayCol':'')+'" data-op="'+X(op)+'" data-date="'+date+'">'+cellHtml(sc,sh)+'</div>'}});return h+'</div>'}
function mount(){
  ...
  E('oa16Refresh').onclick=function(){load(true)};
...
function exportRows(){
  ...
  (S.data.operators||[]).filter(function(o){return String(o&&o.username||'').trim().toUpperCase()!=='OTTICA VISUAL CARE'}).forEach(function(o){var r=[o.username];for(var i=1;i<=days;i++){var dt=parseMonth(S.month);dt.setDate(i);var sc=schedule(o.username,ymd(dt)),sh=storeForDate(dt);if(!sc)r.push(sh.active===false?'Chiuso':fmtStore(sh).replace(/<br>/g,' / '));else if(sc.status==='work'){var a=[];if(sc.start_time&&sc.end_time)a.push(sc.start_time+'-'+sc.end_time);if(sc.start_time_2&&sc.end_time_2)a.push(sc.start_time_2+'-'+sc.end_time_2);r.push(a.join(' / ')||'Turno')}else r.push(statusName(sc.status))}rows.push(r)});
  return rows
}
