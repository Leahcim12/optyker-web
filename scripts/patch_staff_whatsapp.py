...
old_send="async function sendMsg(){const t=$('msgText'),b=$('sendBtn'),m=t.value.trim();if(!m||!selectedThread)return;b.disabled=true;try{await api('send',{client_id:selectedThread,message:m});t.value='';await openThread(selectedThread);t.focus()}catch(e){alert(e.message||'Invio non riuscito')}finally{b.disabled=false}}"
new_send="async function sendMsg(){const t=$('msgText'),b=$('sendBtn'),m=t.value.trim();if(!m||!selectedThread)return;b.disabled=true;try{if(chatMode==='whatsapp')await waApi('send',{client_id:selectedThread,message:m});else await api('send',{client_id:selectedThread,message:m});t.value='';await openThread(selectedThread);t.focus()}catch(e){alert(e.message||'Invio non riuscito')}finally{b.disabled=false}}"
