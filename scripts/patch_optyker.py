...
(function(){function fix(){var r=document.querySelector('.topbarRight'),n=document.getElementById('optykerTopNewClientBtn');if(!r||!n)return;var d=document.getElementById('optykerTopDashboardBtn');if(!d){d=document.createElement('button');d.id='optykerTopDashboardBtn';d.type='button';d.textContent='Dashboard';d.onclick=function(){if(window.showDashboard)showDashboard()};}var old=document.getElementById('optykerCashBtn');if(old&&old!==n)old.remove();n.innerHTML='<span aria-hidden="true">€</span> Cassa';n.title='Cassa per vendite occasionali';n.onclick=function(ev){if(ev){ev.preventDefault();ev.stopPropagation()}if(window.openOptykerCash)window.openOptykerCash('')};if(d.parentNode!==r||d.nextElementSibling!==n)r.insertBefore(d,n)}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',fix);else fix();setInterval(fix,500)})();
</script>\n''')
