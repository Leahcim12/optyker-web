...
<div class="oaV10F"><label>Operatore</label><select id="oaV10Operator"></select></div>
...
<div class="oaV10Availability"><div class="oaV10Label">Fascia oraria disponibile</div><div class="oaV10Help">Ogni orario compare una sola volta. Dopo l’orario scegli lo studio libero.</div><div id="oaV10Times" class="oaV10Times"></div><div id="oaV10StudioWrap" style="display:none;margin-top:10px"><div class="oaV10Label">Studio disponibile</div><div id="oaV10Studios" class="oaV10Studios"></div></div></div>
</div>
...
function getVisibleItem(id){var btn=document.querySelector('.oaEvent[data-id="'+CSS.escape(String(id))+'"]');if(!btn)return null;var all=window.__OPTYKER_APPOINTMENTS_V10_CACHE||[];return all.find(function(x){return String(x.id)===String(id)})||null}
function refreshCache(){if(!E('oaCalendar'))return;var ids=[].slice.call(E('oaCalendar').querySelectorAll('.oaEvent[data-id]')).map(function(b){return b.dataset.id});if(!ids.length)return;var from=new Date();from.setMonth(from.getMonth()-2);var to=new Date();to.setMonth(to.getMonth()+12);staffApi('list',{from:from.toISOString(),to:to.toISOString()}).then(function(x){window.__OPTYKER_APPOINTMENTS_V10_CACHE=x.data||[];bindEvents()}).catch(function(){})}
function bindEvents(){var all=window.__OPTYKER_APPOINTMENTS_V10_CACHE||[];document.querySelectorAll('#oaCalendar .oaEvent[data-id]').forEach(function(b){b.onclick=function(ev){ev.preventDefault();ev.stopPropagation();var a=all.find(function(x){return String(x.id)===String(b.dataset.id)});if(a)openItem(a)}})}
...
function save(){if(!M.item)return;var p={service_id:E('oaV10Service').value,first_name:E('oaV10First').value.trim(),last_name:E('oaV10Last').value.trim(),email:E('oaV10Email').value.trim(),phone:E('oaV10Phone').value.trim(),notes:E('oaV10Notes').value};if(!p.first_name||!p.last_name||!p.email||!p.phone){msg('Nome, cognome, email e telefono sono obbligatori.',true);return}if(!M.selected){msg('Seleziona una fascia oraria e, se richiesto, lo studio.',true);return}p.starts_at=M.selected.starts_at;p.studio_id=M.selected.studio_id||null;p.operator_username=M.selected.operator_username||E('oaV10Operator').value;msg('Salvataggio…');rpc('optyker_appointment_staff_reschedule',{p_username:OPTYKER_CLOUD.username,p_appointment_id:M.item.id,p_payload:p}).then(function(){msg('Modifiche salvate.');setTimeout(function(){E('oaManageModal').classList.remove('open');if(window.optykerOpenAppointments)window.optykerOpenAppointments();setTimeout(refreshCache,350)},350)}).catch(function(e){msg(e.message,true)})}
function cancel(){if(!M.item||!confirm('Annullare questo appuntamento? Rimarrà nello storico del cliente.'))return;msg('Annullamento…');staffApi('appointment_status',{id:M.item.id,status:'cancelled'}).then(function(){msg('Appuntamento annullato.');setTimeout(function(){E('oaManageModal').classList.remove('open');if(window.optykerOpenAppointments)window.optykerOpenAppointments();setTimeout(refreshCache,350)},300)}).catch(function(e){msg(e.message,true)})}
E('oaV10Close').onclick=function(){E('oaManageModal').classList.remove('open')};E('oaV10Save').onclick=save;E('oaV10Cancel').onclick=cancel;['oaV10Service','oaV10Date','oaV10Operator'].forEach(function(i){E(i).onchange=function(){M.selected=null;loadSlots(false)}});E('oaManageModal').addEventListener('click',function(ev){if(ev.target===E('oaManageModal'))E('oaManageModal').classList.remove('open')});
var cal=E('oaCalendar');if(cal){new MutationObserver(function(){setTimeout(refreshCache,0)}).observe(cal,{childList:true,subtree:true})}setTimeout(refreshCache,700);
})();</script>'''
