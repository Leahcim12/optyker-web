...
<div class="oaV10F wide oaV10Private"><label>Note private · solo utenti Optyker con login</label><textarea id="oaV10PrivateNotes" placeholder="Queste note non vengono mostrate al cliente"></textarea></div>
...
function openById(id){if(!id)return;msg('Caricamento appuntamento…');staffManage('get',{id:id}).then(function(x){openItem(x.data||x)}).catch(function(e){msg('');alert(e.message)})}window.optykerOpenAppointmentById=openById
...
function save(){if(!M.item)return;var p={id:M.item.id,service_id:E('oaV10Service').value,status:E('oaV10State').value,first_name:E('oaV10First').value.trim(),last_name:E('oaV10Last').value.trim(),email:E('oaV10Email').value.trim(),phone:E('oaV10Phone').value.trim(),notes:E('oaV10Notes').value,private_notes:E('oaV10PrivateNotes').value};if(!p.first_name||!p.last_name||!p.email||!p.phone){msg('Nome, cognome, email e telefono sono obbligatori.',true);return}if(!M.selected){msg('Seleziona una fascia oraria e, se richiesto, lo studio.',true);return}p.starts_at=M.selected.starts_at;p.studio_id=M.selected.studio_id||null;msg('Salvataggio…');staffManage('reschedule',p).then(function(){msg('Modifiche salvate.');setTimeout(function(){E('oaManageModal').classList.remove('open');if(window.optykerOpenAppointments)window.optykerOpenAppointments()},250)}).catch(function(e){msg(e.message,true)})}
