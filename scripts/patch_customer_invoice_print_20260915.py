...
MARK='OPTYKER_CUSTOMER_INVOICE_PRINT_20260915'
...
row_old='<td><span class=\\"optykerDocsStatus \' +statusClass(r.sdi_status)+\'\\">\' +esc(r.sdi_status||\'—\')+\'</span></td></tr>\'});'
# The generated inline JS has no spaces around concatenation. Keep an exact second form below.
row_old_exact='<td><span class=\\"optykerDocsStatus \' +statusClass(r.sdi_status)+\'\\">\' +esc(r.sdi_status||\'—\')+\'</span></td></tr>\'});'

# Literal strings as produced by patch_documents_nav.py.
literal_old="<td><span class=\\\"optykerDocsStatus '+statusClass(r.sdi_status)+'\\\">'+esc(r.sdi_status||'—')+'</span></td></tr>'});"
literal_new="<td><span class=\\\"optykerDocsStatus '+statusClass(r.sdi_status)+'\\\">'+esc(r.sdi_status||'—')+'</span></td><td><button class=\\\"optykerInvoicePrintBtn\\\" type=\\\"button\\\" data-invoice-print=\\\"'+esc(r.id)+'\\\">Visualizza / Stampa</button></td></tr>'});"
...
for path in HTMLS:
    ...
    # Remove any previous loader so a layered build cannot leave stale assets.
...
if 'fic_send' in SOURCE.read_text(encoding='utf-8'):
