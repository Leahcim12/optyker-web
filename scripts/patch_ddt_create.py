...
MARK="OPTYKER_DDT_CREATE_V1"
if MARK in s:
    raise SystemExit(0)

css=r'''
<style id="optykerDdtCreateCss">
/* OPTYKER_DDT_CREATE_V1 */
...
.optykerDdtClientSearch{display:grid;grid-template-columns:minmax(220px,1fr) minmax(280px,1.5fr);gap:8px}
...
(function(){/* OPTYKER_DDT_CREATE_V1 */
  ...
  var D={clients:[],selectedClient:null,lastSaved:null};
  ...
  function applyClient(id){var c=D.clients.find(function(x){return String(x.id)===String(id)});D.selectedClient=c||null;if(!c)return;E('ddtCustomerName').value=clientName(c);E('ddtVat').value=c.vat||'';E('ddtFiscal').value=c.fiscal||'';E('ddtDestination').value=clientAddress(c)}
  ...
  function ensureModal(){
    ...
    m.querySelector('.optykerDdtClose').onclick=closeModal;E('ddtCancel').onclick=closeModal;E('ddtAddItem').onclick=function(){addItem({})};E('ddtClientSearch').oninput=function(){populateClientSelect(this.value)};E('ddtClientSelect').onchange=function(){applyClient(this.value)};E('ddtSave').onclick=function(){save(false)};E('ddtSavePrint').onclick=function(){save(true)};m.onclick=function(ev){if(ev.target===m)closeModal()}
  }
  function resetForm(){D.selectedClient=null;E('ddtDate').value=today();E('ddtCustomerName').value='';E('ddtVat').value='';E('ddtFiscal').value='';E('ddtDestination').value='';E('ddtReason').value='Vendita';E('ddtStatus').value='draft';E('ddtCarrier').value='';E('ddtPackages').value='1';E('ddtWeight').value='';E('ddtNotes').value='';E('ddtClientSearch').value='';E('ddtItems').innerHTML='';addItem({});populateClientSelect('')}
  function openModal(){ensureModal();resetForm();E('optykerDdtCreateModal').classList.add('open');loadClients()}
  function closeModal(){var m=E('optykerDdtCreateModal');if(m)m.classList.remove('open')}
  function loadClients(){api('list_clients',{}).then(function(x){D.clients=Array.isArray(x.data)?x.data:[];populateClientSelect('')}).catch(function(e){toast('Impossibile caricare i clienti: '+e.message,'error')})}
  function payload(){return {client_id:D.selectedClient&&D.selectedClient.id||'',customer_name:E('ddtCustomerName').value.trim(),customer_vat:E('ddtVat').value.trim(),customer_fiscal_code:E('ddtFiscal').value.trim(),destination:E('ddtDestination').value.trim(),document_date:E('ddtDate').value||today(),reason:E('ddtReason').value,carrier:E('ddtCarrier').value.trim(),packages:Number(E('ddtPackages').value||1),weight:E('ddtWeight').value,status:E('ddtStatus').value,notes:E('ddtNotes').value.trim(),items:collectItems()}}
...
for req in [MARK,'optykerDdtNewBtn','Nuovo documento di trasporto','create_ddt','Salva e stampa']:
    if req not in s:
