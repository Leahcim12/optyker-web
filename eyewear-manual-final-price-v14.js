...
window.OPTYKER_EYEWEAR_MANUAL_FINAL_PRICE_V14='20260916-finalprice1';
...
function ensureManualFinalUi(){
  ...
  if(!box){
    ...
    input.addEventListener('input',function(){var n=manualFinalNumber(this.value);this.dataset.manual=n==null?'0':'1';this.dataset.clear='0';if(n!=null)renderSummary()});
    input.addEventListener('change',function(){var n=manualFinalNumber(this.value);if(n==null){this.dataset.manual='0';this.dataset.clear=this.dataset.persisted==='1'?'1':'0'}else{this.value=n.toFixed(2);this.dataset.manual='1';this.dataset.clear='0'}renderSummary()});
    reset.onclick=function(){var i=manualFinalInput();if(!i)return;i.dataset.manual='0';i.dataset.clear=i.dataset.persisted==='1'?'1':'0';i.value='';renderSummary()};
...
function saveManualFinalPrice(sheetId,p){
  ...
  return fetch(EYE_FINAL_API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json().catch(function(){return {}}).then(function(x){if(!r.ok||!x||x.ok===false)throw new Error(x&&x.error||('HTTP '+r.status));input.dataset.persisted=hasManual?'1':'0';input.dataset.clear='0';return x})})
}
...
setTimeout(function(){ensureManualFinalUi();renderSummary()},120);
