...
echo OPTYKER - CORREZIONE CHIUSURA FINALE FIDELITY V1
...
try {
  ...
  if(-not $src.Contains('function Complete-FidelityTail')){
    ...
    $helper='function Complete-FidelityTail($entry) {'+${function:Complete-FidelityTail}.ToString()+"}`n"
  ...
  if($err.Count){throw 'Verifica aggiornamento fallita.'}
