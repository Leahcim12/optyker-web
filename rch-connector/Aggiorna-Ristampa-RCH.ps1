...
try {
  ...
  $sourceUrl='https://leahcim12.github.io/optyker-web/rch-connector/rch-optyker-connector.ps1?v=20260914-reconcile1'
  ...
  if(-not $block -or -not $block.Contains('function Reconcile-VerifiedReceipt')){throw 'Aggiornamento ristampa non ancora disponibile. Riprova tra qualche minuto.'}
  ...
  if(-not $src.Contains("`$request.path -eq '/receipt/reconcile'")){
    $anchor="} elseif(`$request.method -eq 'POST' -and `$request.path -eq '/receipt/status'){"
    if(-not $src.Contains($anchor)){throw 'Endpoint del connettore non riconosciuti.'}
    $route="} elseif(`$request.method -eq 'POST' -and `$request.path -eq '/receipt/reconcile'){`r`n        Json-Response `$stream 200 (Reconcile-VerifiedReceipt (`$request.body | ConvertFrom-Json))`r`n      "
    $src=$src.Replace($anchor,$route+$anchor)
  }
  if(-not $src.Contains('reconcileReceipt=$true')){$src=$src.Replace('reprintReceipt=$true;','reconcileReceipt=$true;reprintReceipt=$true;')}
  ...
  for($i=0;$i -lt 20;$i++){
    ...
    try{$h=Invoke-RestMethod -Uri 'http://127.0.0.1:8765/health' -TimeoutSec 2;if($h.capabilities.reconcileReceipt -eq $true){$ok=$true;break}}catch{}
  }
  if(-not $ok){throw 'Aggiornamento salvato. Riavvia il PC per avviare il connettore aggiornato.'}
  Write-Host 'Aggiornamento verifica scontrini installato. Ricarica Optyker.' -ForegroundColor Green
  if($pending -gt 0){
    ...
    Write-Host 'Apri Emissione / esito RCH e premi Sincronizza documento verificato. Poi potrai proseguire con Rivedi annullo.' -ForegroundColor Yellow
