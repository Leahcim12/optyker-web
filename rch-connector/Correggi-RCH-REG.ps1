...
trap {
  ...
  Write-Host ''
  Write-Host 'La correzione non esegue chiusure fiscali e non ripete scontrini.' -ForegroundColor Yellow
...
function Read-RchState {
  ...
  foreach($n in @('errorCode','printerError','paperEnd','coverOpen','busy')){
    $node=$r.SelectSingleNode($n)
    if(-not $node -or [int]$node.InnerText -ne 0){throw "RCH non pronta: $n non e zero."}
  }
  $last=$r.SelectSingleNode('lastCmd')
  return [pscustomobject]@{mode=[string]$st.mode;idleState=[string]$st.idleState;lastCmd=if($last){[int]$last.InnerText}else{-1}}
}
...
function Patch-Connector([string]$path){
  ...
  if($text -notmatch '\$ConnectorVersion\s*=\s*''1\.8-auto-receipt'''){throw 'Versione del connettore non riconosciuta: aggiornamento sospeso.'}

  $readPattern='(?s)function Read-PrintedReceipt\(\$document\) \{.*?\r?\n\}\r?\nfunction Assert-WindowsFiscalPlatform'
...
function Assert-WindowsFiscalPlatform
'@
  $text=[regex]::Replace($text,$readPattern,[System.Text.RegularExpressions.MatchEvaluator]{param($m)$replacement},1)
...
# Una sola uscita da Z, esclusivamente se il registratore dichiara Z + inattivo.
Restore-RegIfIdle

# Il diario fiscale rimane intatto. Non riavviare durante claiming/sending.
...
if(Test-Path -LiteralPath $journal){
  foreach($f in Get-ChildItem -LiteralPath $journal -Filter '*.json'){
    try{
      ...
      if($j.state -in @('claiming','sending')){$active=$true}
    }catch{$active=$true}
  }
}
if($active){
...
Write-Host 'Il numero del documento resta da confermare dalla stampa in Emissione / esito RCH.' -ForegroundColor Yellow
