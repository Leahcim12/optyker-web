...
function openDeposits(){
  ...
  api('open_deposits',{client_id:S.clientId}).then(function(x){
    ...
    box.innerHTML=a.map(function(r){
      ...
      return '<div class="optykerCashDepositRow"><div class="optykerCashDepositInfo"><b>'+esc(r.shopify_order_name||'Vendita')+' · '+esc(cl)+'</b><span>'+esc(dt)+' · Totale '+esc(euro(r.total))+' · Pagato '+esc(euro(r.paid_amount))+'</span></div><div class="optykerCashDepositDue"><span>Da saldare</span><b>'+esc(euro(r.due_amount))+'</b></div><div class="optykerCashDepositActions"><select data-settle-pay aria-label="Pagamento del saldo"><option value="">Pagamento del saldo…</option><option value="cash">Contanti</option><option value="card">Carta</option></select><button type="button" data-settle="balance" data-sale="'+esc(r.id)+'">Saldo</button><button type="button" data-settle="delivery_balance" data-sale="'+esc(r.id)+'">Saldo consegna</button></div></div>'
    }).join('');
    var bs=box.querySelectorAll('[data-settle]');for(var i=0;i<bs.length;i++)bs[i].onclick=function(){var pay=this.closest('.optykerCashDepositRow').querySelector('[data-settle-pay]').value;if(!pay){toast('Scegli Contanti o Carta per il saldo prima di stampare lo scontrino.','error');return}var button=E('optykerCashOverlay').querySelector('[data-pay="'+pay+'"]');if(!button){toast('Metodo di pagamento non disponibile.','error');return}button.click();settleExisting(this.getAttribute('data-sale'),this.getAttribute('data-settle'),m)}
...
function loadHistory(){
  ...
  return api('recent_sales',{client_id:clientId,offset:offset,include_deleted:H.includeDeleted}).then(function(x){
    ...
    box.innerHTML=a.length?a.map(function(r){
      ...
      return '<div class="optykerCashSaleRow'+(r.hidden_at?' deleted':'')+'"><div class="optykerCashSaleTop"><div class="optykerCashSaleName">'+esc(r.shopify_order_name||'Vendita')+'</div><div class="optykerCashSaleAmount">'+esc(euro(r.total))+'</div></div><div class="optykerCashSaleMeta">'+esc(receiptDate(r.created_at)+' · '+stageLabel(r.payment_stage||'balance')+' · '+paymentLabel(r.payment_method)+extra)+'</div><div class="optykerCashHistoryActions"><button type="button" data-fiscal-sale="'+esc(r.id)+'">Emissione / esito RCH</button><button type="button" data-receipt="'+esc(r.id)+'">Ristampa su RCH RT</button><button type="button" data-visibility="'+esc(r.id)+'" data-hidden="'+(r.hidden_at?'false':'true')+'">'+(r.hidden_at?'Ripristina':'Elimina dalla scheda')+'</button></div></div>'
    ...
    box.querySelectorAll('[data-receipt]').forEach(function(b){b.onclick=function(){window.OPTYKER_RCH_REPRINT?window.OPTYKER_RCH_REPRINT.openSale(b.getAttribute('data-receipt')):alert('Ricarica Optyker per attivare la ristampa RCH.')}});
