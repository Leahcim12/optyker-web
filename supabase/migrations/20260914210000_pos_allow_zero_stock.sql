CREATE OR REPLACE FUNCTION public.optyker_apply_pos_inventory_movements(p_sale_id uuid, p_lines jsonb, p_operator text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  loop
    ...
    if v_item.category = 'services' then
      continue;
    end if;
    ...
    -- POS can sell stock not yet loaded; preserve non-negative physical stock.
    ...
    insert into public.optyker_inventory_movements(
      ...
      jsonb_build_object('zero_price',v_price = 0,'manual_price',true,'missing_quantity',greatest(0,v_qty-v_before))
