create or replace function public.optyker_complete_client_cart_sale(
  p_sale_id uuid,
  p_operator text
) returns jsonb
language plpgsql
...
begin
  ...
  -- Acconto e saldo sono stati finanziari. La riga del prodotto resta nel
  -- carrello fino alla consegna fisica, indipendentemente da chi richiama
  -- questa funzione. Solo delivered_at autorizza il consumo del carrello.
