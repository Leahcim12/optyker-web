...
export async function makePreview(lib,values,meta){
 ...
 function newPage(){page=pdf.addPage([W,H]);y=H-26;page.drawImage(logo,{x:L,y:y-80,width:80,height:80});draw('Ottica Visual Care',L+96,y-20,16,bold);for(const [i,t] of [values.issuer_address,'Registrazione Ministero della Salute n. '+values.issuer_ministry].entries()){const a=wrap(t,CW-100,10);a.forEach((v,k)=>draw(v,L+96,y-40-i*24-k*11,10));}y-=91;page.drawLine({start:{x:L,y},end:{x:W-L,y},thickness:1,color:rule});y-=20;}
 ...
 ensure(128);paragraph('Data effettiva di consegna: '+values.delivery_date+' | Dichiarazione: '+values.declaration_date,9,bold);const sigPage=pdf.getPageCount()-1,signatureY=y-75;
 const boxes=[{role:'operator',x:L,y:signatureY,width:248,height:57},{role:'client',x:L+280,y:signatureY,width:248,height:57}];
 draw('Firma per il fabbricante',L,y,9,bold);draw('Firma per ricezione',L+280,y,9,bold);y-=13;draw(values.operator_name+' - '+values.operator_capacity,L,y,8);draw(values.signer_name,L+280,y,8);
