// Called only AFTER existing staff/admin authentication.
// Fiscal close is a cashier capability, not a billing-administration session.
const UUID=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
export async function sessionAction(db:any,action:string,p:any,operator:string,canCloseFiscal:boolean){
 ...
 const outcome=async(result:any)=>{
  if(result?.state!=='completed'||!result.operation_id)return result;
  const op=await get(db.from('optyker_cash_session_operations').select('id,kind,state,input,snapshot,command_id').eq('id',result.operation_id).maybeSingle());
  if(!op||op.state!=='completed')throw new Error('Esito della chiusura non disponibile: usa Verifica esito');
  const requested=op.kind==='close'&&op.input?.fiscal===true;
  const confirmed=requested&&op.snapshot?.closure?.fiscal_closure===true&&!!op.command_id;
 ...
 if(action==='session_result'){
  ...
  if(op.input?.fiscal===true&&!canCloseFiscal)throw new Error('Operatore non abilitato alla chiusura RCH');
  return await outcome(await rpc('optyker_cash_session_finish',{p_request_id:p.request_id}));
 }
 ...
 if(action==='session_open'){input.opening_cash=amount(p.opening_cash);input.opening_checks=amount(p.opening_checks??0)}
  ...
  if(p.fiscal===true&&!canCloseFiscal)throw new Error('Operatore non abilitato alla chiusura RCH');
  input.fiscal=p.fiscal===true;
 }
 return await outcome(await rpc('optyker_cash_session_change',{p_request_id:p.request_id,p_business_date:date,p_kind:action==='session_open'?'open':'close',p_operator:operator,p_expected_token:p.expected_token,p_input:input}));
}
