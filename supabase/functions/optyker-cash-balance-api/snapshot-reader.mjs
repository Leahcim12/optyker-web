...
export async function readRecordedBalancesFrom(db,value){
 ...
 // No authentication or readiness result is cached. Called only after the existing auth guard.
 const [active,cart,sales]=await Promise.all([
  ...
  id?read(db.from('optyker_pos_sales').select('id,client_id,status,total,paid_amount,due_amount,delivered_at,created_at,data').eq('client_id',id).is('delivered_at',null).eq('data->>client_cart_selection','true').order('created_at',{ascending:false}).limit(301)):Promise.resolve([])
 ]);
