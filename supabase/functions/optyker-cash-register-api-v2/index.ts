...
Deno.serve(async req=>{
 if(req.method==="OPTIONS")return new Response(null,{status:204,headers:CORS});if(req.method!=="POST")return out({ok:false,error:"METHOD_NOT_ALLOWED"},405);
 try{
  ...
  if(action==="checkout"){const op=await auth(body),lines=Array.isArray(p.lines)?p.lines:[],missing=await missingStock(lines),lottery=cleanLottery(p.lottery_code);if(missing.length||lottery)return out({ok:true,data:await customCheckout(body,op,missing),release:RELEASE});const x=await proxy(body),sale=await postZeroSale(body,x.data);return out({...x,data:sale,release:RELEASE})}
