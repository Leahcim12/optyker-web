...
async function prepare(p,operator){
 ...
 const input={not_already_issued:true,...(payment.payment_method==='mixed'?{payment_breakdown:payment.data?.payment_breakdown}:{}),ts_requested:old.tsRequested===true,talking_receipt:old.talkingReceipt===true,opposition:old.opposition===true,lines:old.lines.map(l=>({department:l.department,quantity:l.quantity,unit_price:l.unitPriceCents/100,description:l.description,expense_code:l.expenseCode||'none'}))};
