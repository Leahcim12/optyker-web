...
async function ready(){
 ...
 if(h){if(h.capabilities?.automaticVoidReference!==true)throw new Error('Aggiornamento annulli richiesto sul PC cassa.');var s=await post(LOCAL+'/status',null,5000);checkStatus(s);return 'local'}
 var d=(await api('status',{},true)).data;
 if(!d.online)throw new Error('PC cassa offline. La pratica resta salvata.');
 if(d.status?.automaticVoidReference!==true)throw new Error('Aggiornamento annulli richiesto sul PC cassa.');
...
function draw(id,s){
 ...
 panel.querySelector('[data-uv-body]').innerHTML='<p>Scontrino <b>'+esc(j.document_number)+'</b> del '+esc(j.document_date)+' · <b>'+esc(money(j.total))+'</b></p><p><b>'+esc(s.message)+'</b></p><p>TS: '+esc(!q?'Non richiesto':q.state==='voided'||q.state==='ts_cancelled'?'Gestito':q.state)+'</p><p>RCH: '+esc(v?.state==='completed'?'Annullo '+v.document_number+' del '+v.document_date:v?.state||'In attesa')+'</p><p>Pagamenti, rimborsi e ordini restano separati. Originale e ricevute vengono conservati.</p>';
...
async function open(id){
 ...
 panel.innerHTML='<div class="optykerCashModalCard" style="max-width:720px"><div class="optykerCashModalTitle">Annulla scontrino — TS + RCH</div><div data-uv-body>Recupero automatico dei dati…</div><p data-uv-message role="status" aria-live="polite"></p><p><a href="/rch-connector/Installa-RCH-Optyker.bat?v=20260915-unified1" download>Aggiorna connettore sul PC cassa (solo la prima volta)</a></p><button type="button" data-uv-close>Chiudi</button></div>';
