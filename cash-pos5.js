...
function openDeliveryBalance(saleId){var b=E('optykerCashDepositsBtn');if(!b){notify('Apri Acconti aperti e seleziona Saldo consegna.','error');return}b.click();var tries=0,t=setInterval(function(){tries++;var x=document.querySelector('[data-sale="'+CSS.escape(saleId)+'"][data-settle="delivery_balance"]');if(x){clearInterval(t);x.closest('.optykerCashDepositRow').querySelector('[data-settle-pay]').focus();notify('Scegli Contanti o Carta per il saldo, poi premi Saldo consegna.','ok')}else if(tries>30){clearInterval(t);notify('Vendita non trovata tra gli acconti aperti.','error')}},100)}
