...
/* OPTYKER_SHOP_OFFICIAL_LOGO_V1 */
(function(){
  // Use the same original OVC logo as the app Home and login, not a CSS imitation.
  const SHOP_BRAND_LOGO='https://cdn.shopify.com/s/files/1/0917/4289/6503/files/visual-care-logo-app-original.png?v=1787903859';
  ...
  function decorateShop(){
    ...
    if(!page.querySelector('.refShopChrome')){
      ...
      chrome.innerHTML=`<button class="refShopHomeBtn" onclick="openShopHome()" aria-label="Ottica Visual Care · Torna allo Shop"><img class="refShopOfficialLogo" src="${SHOP_BRAND_LOGO}" alt="Logo Ottica Visual Care" width="44" height="44" loading="eager" decoding="async" style="display:block;width:44px;height:44px;flex:0 0 44px;object-fit:contain;border:0;border-radius:0;background:transparent;box-shadow:none"><span><b>OTTICA VISUAL CARE</b><small>${title}</small></span></button><div class="refShopChromeActions"><button onclick="goTab('recent')" aria-label="Acquistati di recente" class="refShopHistoryBtn">↺</button><button onclick="goTab('cart')" aria-label="Apri carrello" class="refShopCartBtn">${bagSvg()}<i>${Math.max(0,Number(cartCount?.()||0))}</i></button></div>`;
      page.prepend(chrome);
    }
