...
const APP_VERSION='20260910-staff-reference-v1';
...
self.addEventListener('push',event=>{
  ...
  const options={body:data.body||'Promemoria Optyker',icon:ICON,badge:ICON,tag:data.tag||'optyker-timer',renotify:true,data:{url:data.url||'./?app=13&tab=timer&build='+APP_VERSION}};
...
self.addEventListener('notificationclick',event=>{
  ...
  event.waitUntil((async()=>{
    const list=await self.clients.matchAll({type:'window',includeUncontrolled:true});
    for(const client of list){try{if('navigate' in client)await client.navigate(url);if('focus' in client)return client.focus()}catch{}}
