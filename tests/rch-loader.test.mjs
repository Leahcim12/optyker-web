...
test('production loader copies the module once, before the cash bundle and with the same cache tag',()=>{
  ...
  try {
    ...
    assert.match(out,/rch-preflight.js\?v=20260912-fiscal1/);
    assert.match(out,/cash-register.js\?v=20260912-fiscal1/);
...
test('later Operations patch retains the current cash release, injected logic and alias/manifests',()=>{
  ...
  try {
    ...
    const html='<html data-ovc-ui="2"><head></head><body>Existing page<script src="/rch-preflight.js?v=20260912-fiscal1" id="optykerRchPreflightJs"></script><script src="/cash-register.js?v=20260910-ovc2" id="optykerCashJs"></script></body></html>';
    ...
    const out=readFileSync(join(dir,'_site/index.html'),'utf8');
    assert.equal(out,html.replace('cash-register.js?v=20260910-ovc2','cash-register.js?v=20260912-fiscal1'));
    ...
    assert.equal(cash,injected.replace('20260910-ovc2','20260912-fiscal1'));
