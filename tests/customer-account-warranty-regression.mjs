...
try{
 ...
 for(const [engine,name] of [[chromium,'chromium'],[webkit,'webkit']]){
  ...
  const go=async v=>{const t=page.locator('.ovcDMenuToggle');if(await t.isVisible()&&!(await page.locator('.ovcANav').isVisible()))await t.tap();await page.locator('[data-view='+v+']').tap();};
  try{
   await page.goto('https://otticavisualcare.it/pages/la-mia-scheda-optyker',{waitUntil:'domcontentloaded'});await page.waitForSelector('#ovc-account-native [data-view=eyewear]',{state:'attached'});assert.equal(await page.locator('#ovc-account-native .ovcANav button').count(),9);
   await go('eyewear');await page.waitForSelector('[data-sw-open]');await page.locator('[data-sw-open="'+sid(name==='webkit'?15:14)+'"]').tap();await page.waitForSelector('[data-sw-reason]');assert.equal(await page.locator('[data-sw-reason]').count(),4);await page.screenshot({path:OUT+'/shopify-'+name+'.png'});
   ...
   await go('orders');await page.waitForFunction(()=>document.querySelector('.ovcAPanel')?.textContent.includes('Nessun ordine'));await go('eyewear');await page.waitForSelector('[data-sw-open]');await page.locator('[data-sw-cert="'+sid(9)+'"]').tap();await page.waitForSelector('.swCover a[href^="blob:"]');
   ...
   checks.push(name+': actual Shopify native page scripts, nine navigation buttons, warranty/claim/chat/PDF work with real disposable SQL; existing orders view preserved');
