...
$previous=$previous.Replace('reprintReceipt=$true;','').Replace("'1.8-auto-receipt'","'1.9-pos'")
...
function Invoke-RestMethod {param($Uri,$TimeoutSec) return @{ok=$true;printer='192.168.1.10';capabilities=@{reprintReceipt=$true};mode='REG';idleState='0'}}
...
try{
 ...
 if(-not $actual.Contains("`$request.path -eq '/receipt/reprint'")){throw 'Missing route'}
 & (Join-Path $repo 'rch-connector/Aggiorna-Ristampa-RCH.ps1')
