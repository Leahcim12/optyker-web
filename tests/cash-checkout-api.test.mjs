...
function setup(){
 ...
 const fakeFetch=async(url,options)=>{
  ...
  if(q.includes('CashVariants'))data={nodes:[{id:vid,title:'Default Title',price:'14.50',availableForSale:true,product:{id:'gid://shopify/Product/1',title:'Articolo'}}]};
 ...
 const call=async(payload)=>{const r=await handler(new Request('https://example.test',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'checkout',username:'TEST',password:'TEST PASSWORD',payload})}));return r.json()};
