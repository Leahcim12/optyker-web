...
test('foreign XML must contain the exact original invoice number and date',()=>{
 const doc=makeDocument({...base,series:'foreign',date:'2026-08-20',td:'TD17',entity:{...base.entity,country:'Germania'},original_number:'A&1/26',original_date:'2026-08-30'},{code:'foreign',year:2026,suffix:'/26/A/ES'},info);
 const xml='<TipoDocumento>TD17</TipoDocumento><DatiFattureCollegate><IdDocumento>A&amp;1/26</IdDocumento><Data>2026-08-30</Data></DatiFattureCollegate>';
 assert.doesNotThrow(()=>assertXml(xml,doc));assert.throws(()=>assertXml(xml.replace('A&amp;1/26','OTHER'),doc),/Numero o data/);assert.throws(()=>assertXml(xml.replace('2026-08-30','2026-08-29'),doc),/Numero o data/);
});
