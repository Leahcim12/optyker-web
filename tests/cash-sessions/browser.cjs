// Native cashier UI. All remote calls are intercepted; no real login, payments or RCH commands.
...
(async()=>{
 ...
 for(const scenario of ['staff-desktop','staff-mobile','admin','network-recovery','pending','uncertain','failed','ledger-only','previous-day','cancel-confirmation']){
  ...
  await page.route('**/*',async route=>{
   ...
   if(u.origin===base&&u.pathname==='/session-fixture')return route.fulfill({contentType:'text/html',body:'<!doctype html><html><head></head><body><div id="optykerCashOverlay"><div class="optykerCashHeaderRight"></div></div>'+(admin?'<div id="optykerAdminCashPanel"><div class="optykerAdminCashTop"></div><table><tbody id="optykerAdminCashDays"></tbody></table></div>':'')+'<script>window.OPTYKER_CLOUD={username:"FIXTURE",password:"NOT_A_REAL_PASSWORD"};'+(admin?'sessionStorage.setItem("optyker_billing_admin_token","FIXTURE");':'')+'</script><script src="/cash-sessions.js"></script><script src="/cash-day-control.js"></script>'+(admin?'<script src="/admin-cash-today-controls.js"></script>':'')+'</body></html>'});
   ...
   if(a==='session_close'||a==='session_open'){
    ...
    const r={a,p,polls:0,finished:false};stored.set(p.request_id,r);
    await new Promise(resolve=>setTimeout(resolve,100));
    if(a==='session_open'||!p.fiscal)return reply(finish(r));
    pending={id:p.request_id,state:'pending',fiscal:true};
  ...
  try{
   ...
   if(scenario==='previous-day')assert.equal(await page.locator('#csFiscal').count(),0);
   else assert.equal(await page.locator('#csFiscal').isChecked(),true,'RCH close is enabled by default from the main cashier button');
   if(scenario==='ledger-only')await page.locator('#csFiscal').uncheck();
   await page.locator('#csNotes').fill('Fixture: unified closure');
   await page.evaluate(()=>{document.getElementById('csConfirm').click();document.getElementById('csConfirm')?.click()});
   if(scenario==='cancel-confirmation'){await page.waitForTimeout(250);assert.equal(writes.length,0)}
   else{
    ...
    if(scenario==='uncertain'||scenario==='failed'){
     await page.waitForFunction(()=>/Esito RCH non confermato|RCH offline/.test(document.querySelector('.csMessage')?.textContent));
     assert.ok(!(await page.locator('.csBody').innerText()).includes('RCH confermata'));assert.equal(count,1);assert.equal(writes.length,1);
    }else{
     await page.waitForFunction(()=>document.querySelector('.csMessage')?.textContent.includes('Operazione confermata'));
     assert.equal(writes.length,1);const fiscal=!['ledger-only','previous-day'].includes(scenario);assert.equal(writes[0].p.fiscal,fiscal);
     assert.ok(dialogs[0].includes(fiscal?'NUOVA chiusura fiscale Z':'NON verrà chiusa'));
