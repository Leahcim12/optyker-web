...
(function(){
 ...
 function renderCard(error=''){
   ...
   $('ovcCardTariffs').onclick=()=>window.optykerOpenOvcClientTariffs(C.id,currentName());if($('ovcCardRetry'))$('ovcCardRetry').onclick=()=>loadCard(true);
 ...
 function openTariffs(){return window.OPTYKER_SEPT11.openTariffs();}
 ...
 function ensureWarehouse(){const head=document.querySelector('#warehousePanel .whHeadActions');if(head&&!$('whOvcTariffs')){const b=document.createElement('button');b.id='whOvcTariffs';b.type='button';b.className='whBtn';b.textContent='Tariffe OVC Card';b.onclick=openTariffs;head.prepend(b);}if($('whOvcTariffs'))$('whOvcTariffs').hidden=$('whCategory')?.value!=='services';}
